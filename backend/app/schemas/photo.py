...
class PhotoVerifyRequest(BaseModel):
    ...
    age: int = Field(
        ..., ge=1, le=120, description="Foydalanuvchi yoshi", examples=[25]
    )
    img_b64: str = Field(
        ...,
        description="Rasm base64 formatda (data:image/jpeg;base64,... yoki sof base64)",
        examples=["<base64_string_here>"],
    )
...
class PhotoVerifyResponse(BaseModel):
    ...
    error_messages: list[str] = Field(
        default_factory=list, description="Rad etilish sabablari"
    )
...
class TwoFaceVerifyRequest(BaseModel):
    ...
    ps_img: str = Field(
        ...,
        description="Pasport rasmi base64 formatda",
        examples=["<base64_string_here>"],
    )
    lv_img: str = Field(
        ..., description="Jonli rasm base64 formatda", examples=["<base64_string_here>"]
    )
...
class TwoFaceVerifyResponse(BaseModel):
    ...
    error_messages: list[str] = Field(
        default_factory=list, description="Xatolik xabarlari"
    )
