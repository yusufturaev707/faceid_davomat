"""Davomat — Telegram bot va Mini App uchun umumiy biznes-logika.

Ikki kirish nuqtasi bir xil qoidalar bilan ishlaydi:
  - `endpoints/davomat_bot.py`     — bot serveri (`X-API-Key` + `telegram_id`).
  - `endpoints/davomat_miniapp.py` — Telegram Mini App (imzolangan `initData`).

Endpointlar faqat autentifikatsiya va HTTP qatlamini bajaradi: foydalanuvchini
(`DavomatBot`) aniqlaydi va shu moduldagi funksiyalarni chaqiradi. Region
kesimi, smena/sessiya tegishliligi, `StudentLog` invariantlari va chetlatish
qoidalari faqat shu yerda — ikki kanal o'rtasida tafovut paydo bo'lmasligi uchun.
...
logger = logging.getLogger("faceid.services.davomat_bot")


# ============================================================
# Profil, sessiyalar, region kesimi
# ============================================================


def bot_to_user_response(bot: DavomatBot) -> BotUserResponse:
...
def list_ready_sessions(db: Session) -> list[BotReadySessionResponse]:
...
def resolve_filter_regions(bot: DavomatBot, region_id: int | None) -> set[int]:
    ...
    Bot 2+ regionga biriktirilgan foydalanuvchi region tanlaganda har bir
    so'rovda shu `region_id` ni yuboradi — backend faqat shu region kesimida
    statistika/excel/qidiruv qaytaradi.
    """
...
def ensure_region_assigned(bot: DavomatBot, region_id: int | None) -> int | None:
    """Bot region_id ni yuborgan bo'lsa, biriktirilgan ekanini tekshirish.

    Qaytadi: tasdiqlangan `region_id` yoki `None` (region_id yuborilmagan
    bo'lsa).
    """
    if region_id is None:
        return None
...
def normalize_ps_ser(value: str) -> str:
...
def _lookup_student_in_session(
    ...
    *,
    ...
    `region_id` berilsa, faqat shu region kesimida — foydalanuvchi region
    tanlagan bo'lsa, boshqa regiondagi talaba topilmaydi (xuddi "smenada
    yo'q" kabi xulq).
...
def face_verify(
    ...
    *,
    ...
    Algoritm:
      1) Tanlangan region foydalanuvchiga biriktirilganligi tekshiriladi.
    ...
    region_filter = ensure_region_assigned(bot, region_id)

    # Cyrillic harflarni Latin'ga normalizatsiya qilamiz — Telegram klaviaturasi
    # АД (Cyrillic) yuborib qo'ysa, GTSP topa olmaydi.
    ps_ser_norm = normalize_ps_ser(ps_ser)
    ps_num_norm = (ps_num or "").strip()
    ps_value = f"{ps_ser_norm}{ps_num_norm}"
    imei_value = (jshshir or "").strip()

    if ps_ser_norm != (ps_ser or "").strip().upper():
        logger.info(
    ...
    if current_tss is None or int(current_tss.test_session_id) != int(session_id):
    ...
    matched = _lookup_student_in_session(
        ...
        session_smena_id=int(session_smena_id),
    ...
    already_entered = bool(stu.is_entered)

    # DB ga boshqa murojaat yo'q — GTSP (30s gacha) va inference davomida
    # pool ulanishini band qilib turmaslik uchun o'qish tranzaksiyasini yopamiz.
    db.rollback()
    ...
    # ── 4) Yuz solishtirish ─────────────────────────────────────────────
    try:
        verify_resp, _ps_bgr, _lv_bgr = compare_two_faces(photo_b64, selfie_b64)
    ...
    return BotFaceVerifyResponse(
        ...
        can_attend=bool(verify_resp.verified) and not already_entered,
        fio=fio_gtsp,
        photo_b64=photo_b64,
...
def b64_to_bytes(val: str | None) -> bytes | None:
    """Base64 (sof yoki data URI) → bytes. Yaroqsiz qiymatda `None`."""
    if not val:
        return None
    try:
...
def mark_attendance(
    ...
    *,
    ...
    """Tasdiqlangan talabani davomatga qo'shish.
    ...
    if int(student.session_smena_id) != int(session_smena_id):
        return BotMarkAttendanceResponse(
            status="not_found",
    ...
    selfie_bytes = b64_to_bytes(selfie_b64)
    ...
    verify_score = int(verify_score or 0)
    ...
    try:
        ...
        db.rollback()
        logger.exception("mark-attendance failed: student_id=%s", student_id)
        return BotMarkAttendanceResponse(
            status="error",
            student_id=student.id,
...
def find_by_jshshir(
    ...
    *,
    ...
    """Davomatdan olib tashlash flow uchun talabgorlarni topish.

    Faqat tanlangan smenada va (default) `is_entered=True` bo'lganlarni
    qaytaradi. Bir JShShIR bilan bir nechta yozuv chiqsa, foydalanuvchi
    qaysi birini olib tashlashni tanlaydi.
    """
    region_filter = ensure_region_assigned(bot, region_id)
    ...
    if not rows:
        ...
        # foydalanuvchiga ko'rsatadigan farqlash uchun yana bir tekshiruv.
        if only_entered:
...
def remove_attendance(
    ...
    *,
    ...
    try:
        ...
        db.rollback()
        logger.exception("remove-attendance failed: student_id=%s", student_id)
        return BotRemoveAttendanceResponse(
            status="error",
            student_id=student.id,
...
def list_reason_types(db: Session) -> list[BotReasonTypeInfo]:
...
def list_reasons(db: Session, reason_type_id: int | None) -> list[BotReasonInfo]:
...
def find_for_cheat(
    ...
    *,
    ...
    smenalar/fanlar). Bir nechta bo'lsa, foydalanuvchi tanlaydi.
    """
    region_filter = ensure_region_assigned(bot, region_id)
    ...
    *,
    ...
    Desktop bulk endpoint bilan bir xil semantika:
      - `CheatingLog` yaratadi: `student_id` (UNIQUE), `reason_id`, `user_id`
        (bot kanalida — API key egasi; Mini App'da — sozlangan xizmat
        foydalanuvchisi). `image_path=None` — rasmsiz.
    ...
    # Talabgorning shu sessiyaga tegishliligini tekshirish — ID-spoof himoyasi.
    tss = db.get(TestSessionSmena, student.session_smena_id)
    if tss is None or int(tss.test_session_id) != int(session_id):
    ...
    try:
        ...
        # olinadi — klient bu yerda alohida JShShIR yubormaydi.
        imei = student.imei
        if imei:
        ...
        db.rollback()
        logger.exception(
            "create_cheating failed: student_id=%s reason_id=%s",
            student_id,
            reason_id,
        )
        return BotCheatResponse(
    ...
    logger.info(
        "Chetlatish OK: telegram_id=%s, student_id=%s, reason_id=%s, log_id=%s",
        bot.telegram_id,
        student.id,
        reason.id,
        log.id,
