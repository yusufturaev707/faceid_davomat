...
export default function StudentsPage() {
  ...
  const handleRowClick = async (s: StudentResponse) => {
    ...
    try {
      setSelected(await getStudentApi(s.id));
    } catch {
      /* keep list version */
    } finally {
  ...
  return (
    <div className="flex gap-0">
      {/* Main content */}
      <div
        className={`transition-all duration-300 ${selected ? "flex-1 min-w-0" : "w-full"}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="section-title">Talabgorlar</h2>
            <p className="section-subtitle">
              Talabgorlar ro'yxati va pasport ma'lumotlari
            </p>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50/80 dark:bg-slate-800/80 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <TH>Smena</TH>
                    <TH>Guruh</TH>
                    <TH align="center">Chetlatish</TH>
                    <TH align="center">Keldi</TH>
                  </tr>
                </thead>
                <tbody>
                  {data.items.map((s) => (
                    ...
                    >
                      ...
                      <TD className="font-mono text-gray-500 dark:text-slate-400">
                        {s.test_session_id ?? "—"}
                      </TD>
                      <TD className="text-gray-600 dark:text-slate-400">
                        {s.region_name || "—"}
                      ...
                      <TD className="font-medium text-gray-700 dark:text-slate-300">
                        {s.first_name}
                      </TD>
                      ...
                      <TD className="font-mono text-gray-600 dark:text-slate-400">
                        {s.gr_n || "—"}
                      </TD>
                      <TD align="center">
                        <StatusDot
                          active={s.is_cheating}
                          color="orange"
                          title="Cheating"
                        />
                      </TD>
      ...
      <div
        className={`detail-panel-wrapper flex-shrink-0 ${selected ? "open" : "closed"}`}
      >
        {selected && (
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 detail-photo-animate">
                    {selected.ps_data?.ps_img ? (
                      <img
                        src={
                          selected.ps_data.ps_img.startsWith("data:")
                            ? selected.ps_data.ps_img
                            : `data:image/jpeg;base64,${selected.ps_data.ps_img}`
                        }
                        alt="Pasport rasmi"
                  ...
                  <div className="flex-1 min-w-0 detail-header-text">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-primary-200 hover:text-white transition-colors p-0.5"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
                    ...
                    {selected.subject_name && (
                      <p
                        className="text-sm text-primary-200 mt-2 truncate"
                        title={selected.subject_name}
                      >
                        {selected.subject_name}
                      </p>
                    )}
              ...
              <div className="max-h-[calc(100vh-16rem)] overflow-y-auto">
                ...
                <div className="detail-section px-5 py-4 border-b border-gray-100 dark:border-slate-700">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    <Badge
                      label="Qora ro'yxat"
                      active={selected.is_blacklist}
                      on="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
                    />
                    <Badge
                      label="Cheating"
                      active={selected.is_cheating}
                      on="bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400"
                    />
                    <Badge
                      label="Kirgan"
                      active={selected.is_entered}
                      on="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400"
                    />
                    <Badge
                      label="Tayyor"
                      active={selected.is_ready}
                      on="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                    />
                    <Badge
                      label="Yuz"
                      active={selected.is_face}
                      on="bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400"
                    />
                    <Badge
                      label="Rasm"
                      active={selected.is_image}
                      on="bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400"
                    />
                    <Badge
                      label="Ariza"
                      active={selected.is_applied}
                      on="bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
                    />
                  </div>
                  {selected.is_applied && selected.desc_apply && (
                    <div className="mt-3 px-3 py-2 bg-amber-50 dark:bg-amber-900/15 border border-amber-200 dark:border-amber-800/40 rounded-lg">
                      <p className="text-[10px] uppercase tracking-wider text-amber-600 dark:text-amber-500 font-semibold mb-1">
                        Ariza izohi
                      </p>
                ...
                <div className="detail-section px-5 py-4 border-b border-gray-100 dark:border-slate-700">
                  ...
                  <div className="mt-2.5 space-y-2">
                    ...
                    <DetailRow
                      label="Sessiya ID"
                      value={
                        selected.test_session_id
                          ? String(selected.test_session_id)
                          : null
                      }
                      mono
                    />
                    ...
                    <DetailRow
                      label="Test sanasi"
                      value={formatDate(selected.e_date)}
                    />
                    <DetailRow
                      label="Guruh / Joy"
                      value={`${selected.gr_n || "—"} / ${selected.sp_n || "—"}`}
                      mono
                    />
                    <DetailRow
                      label="S-kod"
                      value={selected.s_code ? String(selected.s_code) : null}
                      mono
                    />
                    ...
                    {selected.lang_id > 0 && (
                      <DetailRow
                        label="Til / Daraja"
                        value={`${selected.lang_id} / ${selected.level_id}`}
                        mono
                      />
                    )}
                  </div>
                ...
                <div className="detail-section px-5 py-4 border-b border-gray-100 dark:border-slate-700">
                  ...
                  {selected.ps_data ? (
                    <div className="mt-2.5 space-y-2">
                      <DetailRow
                        label="Seriya"
                        value={selected.ps_data.ps_ser}
                        mono
                        bold
                      />
                      <DetailRow
                        label="Raqam"
                        value={selected.ps_data.ps_num}
                        mono
                        bold
                      />
                      <DetailRow
                        label="Telefon"
                        value={selected.ps_data.phone}
                      />
                      <DetailRow
                        label="Jinsi"
                        value={selected.ps_data.gender_name}
                      />
                          ...
                          <>
                            <div className="bg-gray-50 dark:bg-slate-800 rounded-lg px-3 py-2 max-h-16 overflow-hidden relative">
                              <p className="text-[10px] font-mono text-gray-500 dark:text-slate-400 leading-tight break-all line-clamp-3">
                                [
                                {selected.ps_data.embedding.length > 200
                                  ? selected.ps_data.embedding.slice(0, 200) +
                                    "..."
                                  : selected.ps_data.embedding}
                                ]
                              </p>
                            ...
                            <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-1">
                              {selected.ps_data.embedding.split(",").length}{" "}
                              o'lcham
                            </p>
                          </>
                        ) : (
                    ...
                    <p className="text-sm text-gray-400 dark:text-slate-500 italic mt-2.5">
                      Pasport ma'lumotlari mavjud emas
                    </p>
                  )}
                </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400">
                ...
                talabgorini va unga tegishli barcha ma'lumotlarni (pasport,
                loglar) o'chirmoqchimisiz?
              </p>
        ...
        <div className="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="px-6 py-5 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <ModalField label="Guruh raqami">
                  <input
                    type="number"
                    value={form.gr_n}
                    onChange={(e) => setField("gr_n", Number(e.target.value))}
                    className="input-field w-full"
                  />
                </ModalField>
                <ModalField label="Joy raqami">
                  <input
                    type="number"
                    value={form.sp_n}
                    onChange={(e) => setField("sp_n", Number(e.target.value))}
                    className="input-field w-full"
                  />
                </ModalField>
                <ModalField label="S-kod">
                  <input
                    type="number"
                    value={form.s_code}
                    onChange={(e) => setField("s_code", Number(e.target.value))}
                    className="input-field w-full"
                  />
                </ModalField>
                <ModalField label="Fan ID">
                  <input
                    ...
                    onChange={(e) =>
                      setField("subject_id", Number(e.target.value))
                    }
                    className="input-field w-full"
                  />
                </ModalField>
                <ModalField label="Til ID">
                  <input
                    ...
                    value={form.lang_id}
                    onChange={(e) =>
                      setField("lang_id", Number(e.target.value))
                    }
                    className="input-field w-full"
                  />
                </ModalField>
                <ModalField label="Daraja ID">
                  <input
                    ...
                    value={form.level_id}
                    onChange={(e) =>
                      setField("level_id", Number(e.target.value))
                    }
                    className="input-field w-full"
                  />
                </ModalField>
              ...
              {editId && (
                <>
                  <p className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mt-6 mb-3">
                    Holat bayroqlari
                  </p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    <ToggleField
                      ...
                      onChange={(v) => setField("is_ready", v)}
                    />
                    <ToggleField
                      label="Yuz (Face)"
                      checked={!!form.is_face}
                      onChange={(v) => setField("is_face", v)}
                    />
                    <ToggleField
                      label="Rasm (Image)"
                      checked={!!form.is_image}
                      onChange={(v) => setField("is_image", v)}
                    />
                    <ToggleField
                      label="Cheating"
                      checked={!!form.is_cheating}
                      onChange={(v) => setField("is_cheating", v)}
                      color="orange"
                    />
                    <ToggleField
                      label="Qora ro'yxat"
                      checked={!!form.is_blacklist}
                      onChange={(v) => setField("is_blacklist", v)}
                      color="red"
                    />
                    <ToggleField
                      label="Kirgan (Entered)"
                      checked={!!form.is_entered}
                      onChange={(v) => setField("is_entered", v)}
                      color="green"
                    />
                    <ToggleField
                      label="Ariza (Applied)"
                      checked={!!form.is_applied}
                      onChange={(v) => setField("is_applied", v)}
                      color="orange"
                    />
                  </div>
                  <div className="mt-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <ModalField label="Pasport seriyasi">
                      <input
                        type="text"
                        value={form.ps_ser || ""}
                        onChange={(e) => setField("ps_ser", e.target.value)}
                        placeholder="AA"
                        maxLength={5}
                        className="input-field w-full"
                      />
                    ...
                    <ModalField label="Pasport raqami">
                      <input
                        type="text"
                        value={form.ps_num || ""}
                        onChange={(e) => setField("ps_num", e.target.value)}
                        placeholder="1234567"
                        maxLength={10}
                        className="input-field w-full"
                      />
                    ...
                    <ModalField label="Telefon raqami">
                      <input
                        type="text"
                        value={form.ps_phone || ""}
                        onChange={(e) => setField("ps_phone", e.target.value)}
                  ...
                  <div className="mt-4 space-y-4">
                    <ModalField label="Rasm (Base64)">
                      ...
                      {form.ps_img && (
                        <div className="mt-2 flex items-center gap-3">
                          <img
                            src={
                              form.ps_img.startsWith("data:")
                                ? form.ps_img
                                : `data:image/jpeg;base64,${form.ps_img}`
                            }
                            alt="Preview"
                    ...
                    <ModalField label="Embedding">
                      <textarea
                        ...
                        onChange={(e) =>
                          setField("ps_embedding", e.target.value)
                        }
...
function TH({
  ...
  return (
    <th
      className={`px-3 py-3 font-semibold text-[11px] uppercase tracking-wider text-gray-400 dark:text-slate-500 whitespace-nowrap ${align === "center" ? "text-center" : "text-left"}`}
    >
      {children}
    </th>
  );
...
function TD({
  ...
  return (
    <td
      className={`px-3 py-2.5 whitespace-nowrap ${align === "center" ? "text-center" : "text-left"} ${className}`}
    >
      {children}
    </td>
  );
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium ${active ? on : "bg-gray-50 text-gray-400 dark:bg-slate-700/50 dark:text-slate-500"}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${active ? "bg-current" : "bg-gray-300 dark:bg-slate-500"}`}
      />
      {label}
    </span>
  );
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs uppercase tracking-wider font-semibold text-gray-400 dark:text-slate-500">
      {children}
    </p>
  );
}
...
function ToggleField({
  ...
  return (
    <label className="flex items-center gap-2.5 cursor-pointer p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
      ...
      >
        <span
          className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform duration-200 ${checked ? "translate-x-4" : "translate-x-0"}`}
        />
      </button>
